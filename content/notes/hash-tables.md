...
# Hash Tables
---
A hash table is a data structure which offers a fast implementation of the associative array (abstract data structure, also called a map, symbol table or dictionary) [[hash-tables#API|API]]. 
...
Array indexing has algorithmic [[big-o-notation|complexity]] `O(1)`, making hash tables fast at storing and retrieving data.

- Hash tables are [[vectors-arrays|vector]] with *n* entries.

Hash functions:
- H(**k**) gives a key, **k**, yields a value in the range [0,n] (it is important to have an even distribution).
- Store key, **k**, and value, **v**, at index H(**k**) in the [[vectors-arrays|vector]].
