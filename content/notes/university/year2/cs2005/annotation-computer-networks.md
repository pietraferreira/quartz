...
>```
>%%
>*%%HIGHLIGHT%% ==As long as the layer provides the sameservice to the layer above it, and uses the same services from the layer below it, the remainder of thesystem remains unchanged when a layer’s implementation is changed.== *
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP provides a ­connection-oriented service to its applications. This service includesguaranteed delivery of application-layer messages to the destination and flow control (that is,sender/receiver speed matching). TCP also breaks long messages into shorter ­segments and providesa congestion-control mechanism== *
...
>```
>%%
>*%%HIGHLIGHT%% ==ThroughputIn Chapter 1 we introduced the concept of available throughput, which, in the context of acommunication session between two processes along a network path, is the rate at which the sendingprocess can deliver bits to the receiving process. Because other sessions will be sharing the bandwidthalong the network path, and because these other sessions will be coming and going, the availablethroughput can fluctuate with time. These observations lead to another natural service that a transport-layer protocol could provide, namely, guaranteed available throughput at some specified rate. With sucha service, the application could request a guaranteed throughput of r bits/sec, and the transport protocolwould then ensure that the available throughput is always at least r bits/sec. Such a guaranteedthroughput service would appeal to many applications. For example, if an Internet telephony applicationencodes voice at 32 kbps, it needs to send data into the network and have data delivered to thereceiving application at this rate. If the transport protocol cannot provide this throughput, the applicationwould need to encode at a lower rate (and receive enough throughput to sustain this lower coding rate)or may have to give up, since receiving, say, half of the needed throughput is of little or no use to thisInternet telephony application. Applications that have throughput requirements are said to bebandwidth-sensitive applications. Many current multimedia applications are bandwidth sensitive,although some multimedia applications may use adaptive coding techniques to encode digitized voice orvideo at a rate that matches the currently available throughput.While bandwidth-sensitive applications have specific throughput requirements, elastic applications canmake use of as much, or as little, throughput as happens to be available. Electronic mail, file transfer,and Web transfers are all elastic applications. Of course, the more throughput, the better. There’sanadage that says that one cannot be too rich, too thin, or have too much throughput!== *
...
>- The encrypted data travels over the network to the TCP socket in the receiving process.
>- The receiving socket passes the encrypted data to SSL, which decrypts it.
>- Finally, SSL passes the decrypted data through its SSL socket to the receiving process.
>%%TAGS%%
>#chatgpt, #key terms
^y6rxqnnvon
...
>```
>%%
>*%%HIGHLIGHT%% ==The  HEAD method is similar to the  GET method. When a server receives a request with the  HEADmethod, it responds with an HTTP message but it leaves out the requested object. Applicationdevelopers often use the  HEAD method for debugging.== *
...
>```
>%%
>*%%HIGHLIGHT%% ==HTTP server is stateless== *
>%%LINK%%[[#^ixs3zxdkhoo|show annotation]]
>%%COMMENT%%
>In the case of the HTTP protocol, being stateless means that each request made by a client to a server is independent and carries all the necessary information for the server to process the request. The server does not retain any knowledge of past requests or client state between different requests.
>%%TAGS%%
>#chatgpt
^ixs3zxdkhoo
...
>```
>%%
>*%%HIGHLIGHT%% ==Cookies, defined in [RFC 6265], allow sites to keep track of users== *
>%%LINK%%[[#^0fdpgq4y1gjf|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==cookie technology has four components: (1) a cookie header line in the HTTPresponse message; (2) a cookie header line in the HTTP request message; (3) a cookie file kept on the== *
>%%LINK%%[[#^pdxpxup762l|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==user’s end system and managed by the user’s browser; and (4) a back-end database at the Web site== *
>%%LINK%%[[#^e238bxz5op6|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the POP3 protocol does not provide any means for a user to create remotefolders and assign messages to folders== *
>%%LINK%%[[#^2fcw86j3jnf|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==An IMAP server will associate each message with a folder; when a message first arrives at the server, itis associated with the recipient’s INBOX folder.== *
>%%LINK%%[[#^qff5aiw4nj|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==unlike POP3, an IMAP server maintains user state information across IMAP sessions—forexample, the names of the folders and which messages are associated with which folders== *
>%%LINK%%[[#^t13hbqx9wym|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Another important feature of IMAP is that it has commands that permit a user agent to obtaincomponents of messages.== *
>%%LINK%%[[#^zjwulaugf3|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==With this service, the user agent isan ordinary Web browser, and the user communicates with its remote mailbox via HTTP== *
>%%LINK%%[[#^z4b6my7jmb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When a sender, such as Alice, wants to send an e-mail message, the e-mail message is sent from herbrowser to her mail server over HTTP rather than over SMTP. Alice’s mail server, however, still sendsmessages to, and receives messages from, other mail servers using SMTP== *
>%%LINK%%[[#^vkuo5cft8wr|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==a typical network applicationconsists of a pair of programs—a client program and a server program—residing in two different endsystems. When these two programs are executed, a client process and a server process are created,and these processes communicate with each other by reading from, and writing to, sockets== *
>%%LINK%%[[#^mcwlbw3jmb9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==There are two types of network applications. One type is an implementation whose operation isspecified in a protocol standard, such as an RFC== *
>%%LINK%%[[#^jzpcj06i23|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The other type of network application is a proprietary network application. In this case the client andserver programs employ an application-layer protocol that has not been openly published in an RFC== *
>%%LINK%%[[#^56xrhr7b98a|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP is connection oriented and provides a reliable byte-streamchannel through which data flows between two end systems. UDP is connectionless and sendsindependent packets of data from one end system to the other, without any guarantees about delivery== *
>%%LINK%%[[#^5f2wgpy8bb4|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When a socket is created, an identifier, called a port number, is assignedto it.== *
>%%LINK%%[[#^qkx6ibte68s|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==attaching the source address to the packet istypically not done by the UDP application code== *
>%%LINK%%[[#^z5zgswdpp7h|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Unlike UDP, TCP is a connection-oriented protocol. This means that before the client and server canstart to send data to each other, they first need to handshake and establish a TCP connection.== *
>%%LINK%%[[#^0timecc58sbj|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the TCP servermust be running as a process before the client attempts to initiate contact. Second, the server programmust have a special door—more precisely, a special socket—that welcomes some initial contact from aclient process running on an arbitrary host== *
>%%LINK%%[[#^bk69gw3nzm|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==From the application’s perspective, the client’s socket and the server’s connection socket are directlyconnected by a pipe== *
>%%LINK%%[[#^ijp25m60a1|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the transport layer is a central piece of the layerednetwork architecture. It has the critical role of providing communication services directly to theapplication processes running on different hosts== *
>%%LINK%%[[#^6b0l7f6frs7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A transport-layer protocol provides for logical communication between application processes runningon different hosts. By logical communication, we mean that from an application’s perspective, it is as ifthe hosts running the processes were directly connected== *
>%%LINK%%[[#^kk21hh9ozyh|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==transport-layer protocols are implemented in the end systems but not innetwork routers. On the sending side, the transport layer converts the application-layer messages itreceives from a sending application process into transport-layer packets, known as transport-layersegments in Internet terminology== *
>%%LINK%%[[#^ipswd2yb1y|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==It’s important to note that network routers act only on the network-layer fields of thedatagram; that is, they do not examine the fields of the transport-layer segment encapsulated with thedatagram.== *
>%%LINK%%[[#^fhbajahpcrl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Whereas atransport-layer protocol provides logical communication between== *
>%%LINK%%[[#^frqr7h1zz7b|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==processes running on different hosts, a network-layer protocol provides logical-communication betweenhosts== *
>%%LINK%%[[#^0ga30pm9gdx|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==application messages  letters in envelopesprocesses  cousinshosts (also called end systems)  housestransport-layer protocol  Ann and Billnetwork-layer protocol  postal service (including mail carriers)== *
>%%LINK%%[[#^w5j241bkzrn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==transport-layer protocols live in the end systems.== *
>%%LINK%%[[#^s95y20yi5r|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==a transport protocol moves messages from application processes to the network edge== *
>%%LINK%%[[#^0qjwg17fcq|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the services that a transport protocol can provide are often constrained by the servicemodel of the underlying network-layer protocol== *
>%%LINK%%[[#^0q5p6yndj2i|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==a transport protocol can offer reliable data transfer service to an application even when theunderlying network protocol is unreliable== *
>%%LINK%%[[#^lo1ddoppg4b|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==a transport protocol can use encryption to guarantee that application messages are not readby intruders== *
>%%LINK%%[[#^b6v4g1lq7pp|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==One of these protocols is UDP (User Datagram Protocol), which provides an unreliable, connectionlessservice to the invoking application. The second of these protocols is TCP (Transmission ControlProtocol), which provides a reliable, connection-oriented service to the invoking application== *
>%%LINK%%[[#^oh6scrvumce|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==we refer to the transport-layer packet as a segment== *
>%%LINK%%[[#^w8wqxs54ee|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The IP service model is a best-effort delivery service. This means thatIP makes its “best effort” to deliver segments between communicating hosts, but it makes noguarantees.== *
>%%LINK%%[[#^a6e6dc9ixhl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==IPis said to be an unreliable service.== *
>%%LINK%%[[#^jnb3q13994c|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Extending host-to-host delivery to process-to-process delivery is called transport-layer multiplexingand demultiplexing== *
>%%LINK%%[[#^sv1l7mnixwb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==a process (as part of a network application) can have one or moresockets== *
>%%LINK%%[[#^hsj8tbixkid|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==At the receivingend, the transport layer examines these fields to identify the receiving socket and then directs thesegment to that socket. This job of delivering the data in a transport-layer segment to the correct socketis called demultiplexing. The job of gathering data chunks at the source host from different sockets,encapsulating each data chunk with header information (that will later be used in demultiplexing) tocreate segments, and passing the segments to the network layer is called multiplexing== *
>%%LINK%%[[#^iysaq6ydhu|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When Bill receives a batch of mail from the mail carrier, heperforms a demultiplexing operation by observing to whom the letters are addressed and then handdelivering the mail to his brothers and sisters. Ann performs a multiplexing operation when she collectsletters from her brothers and sisters and gives the collected mail to the mail person== *
>%%LINK%%[[#^qqgd3o92rsc|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==we know that transport-layer multiplexingrequires (1) that sockets have unique identifiers, and (2) that each segment have special fields thatindicate the socket to which the segment is to be delivered== *
>%%LINK%%[[#^saror3lr9h|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==source port number field and the destination port number field== *
>%%LINK%%[[#^znm5el7a6n8|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==source IP address, source port number, destination IP address,destination port number== *
>%%LINK%%[[#^w7yubkv5rp|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==One subtle difference between a TCP socket and a UDP socket is that a TCP== *
>%%LINK%%[[#^2tat2zz0q86|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==socket is identified by a four-tuple== *
>%%LINK%%[[#^x8fxaomu97q|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==in contrast with UDP, two arriving TCP segments with different source IP addresses orsource port numbers will (with the exception of a TCP segment carrying the original connection-establishment request) be directed to two different sockets== *
>%%LINK%%[[#^60i2146d1tx|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==UDP takes messages from the application process, attaches source and destination port numberfields for the multiplexing/demultiplexing service, adds two other small fields, and passes the resultingsegment to the network layer== *
>%%LINK%%[[#^ifa1l973ad8|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DNS is an example of an application-layer protocol that typically uses UDP. When the DNS applicationin a host wants to make a query, it constructs a DNS query message and passes the message to UDP.Without performing any handshaking with the UDP entity running on the destination end system, thehost-side UDP adds header fields to the message and passes the resulting segment to the networklayer.== *
>%%LINK%%[[#^7f38uxofdm6|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Finer application-level control over what data is sent, and when== *
>%%LINK%%[[#^x2xpaq1tf4|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==No connection establishment== *
>%%LINK%%[[#^34h966p5dzg|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==No connection state== *
>%%LINK%%[[#^or1bwwu4ph|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Small packet header overhead== *
>%%LINK%%[[#^bccwause84g|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==it is possible for an application to havereliable data transfer when using UDP. This can be done if reliability is built into the application itself== *
>%%LINK%%[[#^yb427q32wb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The UDP checksum provides for error detection== *
>%%LINK%%[[#^1izhb153goo|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP is said to be connection-oriented because before one application process can begin to send datato another, the two processes must first “handshake” with each other== *
>%%LINK%%[[#^yquc6q4mh78|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP protocol runs only in the end systems and not in theintermediate network elements (routers and link-layer switches== *
>%%LINK%%[[#^k15gxu6tvc7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A TCP connection provides a full-duplex service: If there is a TCP connection between Process A onone host and Process B on another host, then application-layer data can flow from Process A toProcess B at the same time as application-layer data flows from Process B to Process A. A TCPconnection is also always point-to-point, that is, between a single sender and a single receiver== *
>%%LINK%%[[#^vy878omb3go|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==three-way handshake== *
>%%LINK%%[[#^ncfqg6lrda|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP directs this data to the connection’s send buffer, which is one of the buffers that is set aside duringthe initial three-way handshake== *
>%%LINK%%[[#^b01t4jzhwbq|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP pairs each chunk of client data with a TCP header, thereby forming TCP segments. The segmentsare passed down to the network layer, where they are separately encapsulated within network-layer IPdatagrams.== *
>%%LINK%%[[#^8yz3gxi453j|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Two of the most important fields in the TCP segment header are the sequence number field and theacknowledgment number field. These fields are a critical part of TCP’s reliable data transfer service.== *
>%%LINK%%[[#^wsl4m7ajrxm|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The sequence number for a segment is therefore the byte-streamnumber of the first byte in the segment.== *
>%%LINK%%[[#^qbrag81auj8|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Recallthat TCP is full-duplex, so that Host A may be receiving data from Host B while it sends data to Host B(as part of the same TCP connection== *
>%%LINK%%[[#^e2rzm2utp5f|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The acknowledgment number that Host A puts in its segment isthe sequence number of the next byte Host A is expecting from Host B== *
>%%LINK%%[[#^anqbts7cyz|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Because TCP only acknowledges bytes up to the first missing byte in the stream, TCP issaid to provide cumulative acknowledgments.== *
>%%LINK%%[[#^g67bw9ca4ff|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The sample RTT, denoted  SampleRTT,for a segment is the amount of time between when the segment is sent (that is, passed to IP) and whenan acknowledgment for the segment is received.== *
>%%LINK%%[[#^ko2wrz3ey3|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP creates a reliable data transfer service on top of IP’s unreliable best-effort service.== *
>%%LINK%%[[#^4edm3rvrybl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP receive buffer isuncorrupted, without gaps, without duplication, and in sequence;== *
>%%LINK%%[[#^wtv7uie848l|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==duplicate ACK is an ACKthat reacknowledges a segment for which the sender has already received an earlier acknowledgment== *
>%%LINK%%[[#^y68fvv8pm0e|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP’s error-recovery mechanism is probably bestcategorized as a hybrid of GBN and SR protocols== *
>%%LINK%%[[#^fb28aia0fzd|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==selective acknowledgment [RFC 2018], allows a TCPreceiver to acknowledge out-of-order segments selectively rather than just cumulatively acknowledgingthe last correctly received, in-order segment. When combined with selective retransmission—skippingthe retransmission of segments that have already been selectively acknowledged by the receiver—TCPlooks a lot like our generic SR protocol== *
>%%LINK%%[[#^w4epnh272l8|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP provides a flow-control service to its applications to eliminate the possibility of the senderoverflowing the receiver’s buffer.== *
>%%LINK%%[[#^o2ys3hud5yk|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Flow control is thus a speed-matching service—matching the rate atwhich the sender is sending against the rate at which the receiving application is reading== *
>%%LINK%%[[#^4r1hi3a9oh5|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP provides flow control by having the sender maintain a variable called the receive window.Informally, the receive window is used to give the sender an idea of how much free buffer space isavailable at the receiver== *
>%%LINK%%[[#^x2sohwy30xn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==During the life of a TCP connection, the TCP protocol running in each host makes transitions throughvarious TCP states== *
>%%LINK%%[[#^y39xzlelczh|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP must use end-to-endcongestion control rather than network-assisted congestion control, since the IP layer provides noexplicit feedback to the end systems regarding network congestion== *
>%%LINK%%[[#^aua7v3sy8yu|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==he approach taken by TCP is to have each sender limit the rate at which it sends traffic into itsconnection as a function of perceived network congestion== *
>%%LINK%%[[#^qbbvezz6ud|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The TCP congestion-control mechanismoperating at the sender keeps track of an additional variable, the congestion window. The congestionwindow, denoted  cwnd, imposes a constraint on the rate at which a TCP sender can send traffic intothe network.== *
>%%LINK%%[[#^g9xtsehinuf|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Because TCP uses acknowledgments to trigger (or clock) its increase in congestion window size, TCPis said to be self-clocking.== *
>%%LINK%%[[#^6qv7sbcuz0y|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A lost segment implies congestion, and hence, the TCP sender’s rate should be decreasedwhen a segment is lost.== *
>%%LINK%%[[#^f3ckvej3lgl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==An acknowledged segment indicates that the network is delivering the sender’s segments tothe receiver, and hence, the sender’s rate can be increased when an ACK arrives for apreviously unacknowledged segment== *
>%%LINK%%[[#^x71zksyus1q|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Bandwidth probing== *
>%%LINK%%[[#^138yve5zywak|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%LINK%%[[#^i1h21dmd64d|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==in the slow-start state, the value of  cwnd begins at 1MSS and increases by 1 MSS every time a transmitted segment is first acknowledged== *
>%%LINK%%[[#^nxd5c2b7lj|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==if there is a loss event (i.e., congestion) indicated by a timeout, the TCP sender sets the value ofcwnd to 1 and begins the slow start process anew.== *
>%%LINK%%[[#^3ct00u9alya|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==when the value of  cwnd equals  ssthresh, slow start ends and TCP transitionsinto congestion avoidance mode== *
>%%LINK%%[[#^oqkme12pa4|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==On entry to the congestion-avoidance state, the value of  cwnd is approximately half its value whencongestion was last encountered—congestion could be just around the corner! Thus, rather thandoubling the value of  cwnd every RTT, TCP adopts a more conservative approach and increases thevalue of  cwnd by just a single MSS every RTT== *
>%%LINK%%[[#^aj74h2cmaqn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==In fast recovery, the value of  cwnd is increased by 1 MSS for every duplicate ACK received for themissing segment that caused TCP to enter the fast-recovery state== *
>%%LINK%%[[#^xxsqybtwbn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==TCP congestion control is often referred to as an additive-increase,multiplicative-decrease (AIMD)== *
>%%LINK%%[[#^ces08uywihm|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When running over UDP, applications can pump theiraudio and video into the network at a constant rate and occasionally lose packets, rather than reducetheir rates to “fair” levels at times of congestion and not lose any packets== *
>%%LINK%%[[#^zmkbmkekby|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==extensions to both IP and TCP [RFC 3168] have beenproposed, implemented, and deployed that allow the network to explicitly signal congestion to a TCP== *
>%%LINK%%[[#^cw8z7eambv|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==sender and receiver. This form of network-assisted congestion control is known as Explicit CongestionNotification== *
>%%LINK%%[[#^7mat2cvj37a|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==At the network layer, two bits (with four possible values, overall) in the Type of Service field of the IPdatagram header (which we’ll discuss in Section 4.3) are used for ECN== *
>%%LINK%%[[#^xp1a8afc1a9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The primary role of the network layer is deceptively simple—to move packets from a sending host to areceiving host. To do so, two important network-layer functions can be identified:== *
>%%LINK%%[[#^gu7rujff60e|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Forwarding. When a packet arrives at a router’s input link, the router must move the packet to theappropriate output link. For example, a packet arriving from Host H1 to Router R1 in Figure 4.1 mustbe forwarded to the next router on a path to H2== *
>%%LINK%%[[#^d11d2w52qpb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Routing. The network layer must determine the route or path taken by packets as they flow from asender to a receiver. The algorithms that calculate these paths are referred to as routingalgorithms.== *
>%%LINK%%[[#^macgjxq87np|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Forwarding refers to the router-localaction of transferring a packet from an input link interface to the appropriate output link interface== *
>%%LINK%%[[#^q1rsv58xfgb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Routing refers to the network-wide process that determines the end-to-endpaths that packets take from source to destination== *
>%%LINK%%[[#^5ih1iiuun88|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==hardware== *
>%%LINK%%[[#^78hyiiadr3r|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==software== *
>%%LINK%%[[#^732g2ijonrs|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A key element in every network router is its forwarding table. A router forwards a packet by examiningthe value of one or more fields in the arriving packet’s header, and then using these header values toindex into its forwarding table== *
>%%LINK%%[[#^6jk2cw1vveu|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the routing algorithm determines the contents of the routers’ forwarding tables== *
>%%LINK%%[[#^b9ua37zgk1|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==software-defined networking (SDN), where the network is “software-defined”because the controller that computes forwarding tables and interacts with routers is implemented insoftware== *
>%%LINK%%[[#^67f7uisn6v|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Four router components can beidentified== *
>%%LINK%%[[#^1mphiuyy95k|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Input ports. An input port performs several key functions. It performs the physical layer function ofterminating an incoming physical link at a router== *
>%%LINK%%[[#^x4mtagcvrdn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Switching fabric. The switching fabric connects the router’s input ports to its output ports== *
>%%LINK%%[[#^cnuzoue418a|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Output ports. An output port stores packets received from the switching fabric and transmits thesepackets on the outgoing link by performing the necessary link-layer and physical-layer functions== *
>%%LINK%%[[#^y954dcd4ylo|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Routing processor. The routing processor performs control-plane functions. In traditional routers, itexecutes the routing protocols (which we’ll study in Sections 5.3 and 5.4), maintains routing tablesand attached link state information, and computes the forwarding table for the router. In SDNrouters, the routing processor is responsible for communicating with the remote controller in order to(among other activities) receive forwarding table entries computed by the remote controller, andinstall these entries in the router’s input ports== *
>%%LINK%%[[#^ab8rms6bml|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Destination-based forwarding== *
>%%LINK%%[[#^cp0glqsu217|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Generalized forwarding== *
>%%LINK%%[[#^ev5ojudt1|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the input port’s line-termination function and link-layer processing implement the physical and link layers for that individualinput link== *
>%%LINK%%[[#^z83mwxuv8hb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==With this style of forwarding table, the router matches a prefix of the packet’s destination address withthe entries in the table== *
>%%LINK%%[[#^5sd2ptxqnsn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When there are multiple matches, the router uses the longest prefix matching rule; thatis, it finds the longest matching entry in the table== *
>%%LINK%%[[#^142z5mfwhrlj|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==IPv4 datagram format== *
>%%LINK%%[[#^ll8fcloo8b|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the Internet’s network-layer packet is referred to as a datagram== *
>%%LINK%%[[#^2nyjg42xwzs|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The key fields in the IPv4 datagram are thefollowing:== *
>%%LINK%%[[#^e8jqwbv0luf|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%LINK%%[[#^tcrjfmr8i4|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Header length== *
>%%LINK%%[[#^km9z8kq018f|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Type of service. The type of service (TOS)== *
>%%LINK%%[[#^w4rk7gdu2tf|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Datagram length== *
>%%LINK%%[[#^91l05nvazg7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Identifier, flags, fragmentation offset== *
>%%LINK%%[[#^fuv8mbh4n5u|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Time-to-live. The time-to-live (TTL) field is included to ensure that datagrams do not circulateforever== *
>%%LINK%%[[#^vcao1rg8jx7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==Protocol.== *
>%%LINK%%[[#^t5kx2tq06gm|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Thevalue of this field indicates the specific transport-layer protocol to which the data portion of this IPdatagram should be passed. For example, a value of 6 indicates that the data portion is passed toTCP== *
>%%LINK%%[[#^qdmk11l3nk8|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Header checksum. The header checksum aids a router in detecting bit errors in a received IPdatagram.== *
>%%LINK%%[[#^iw15etcegtr|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Source and destination IP addresses== *
>%%LINK%%[[#^qxqhzqeczjd|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==Options== *
>%%LINK%%[[#^y8bvhmwik4n|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%LINK%%[[#^mllefsnl8vc|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==an IP datagram has a total of 20 bytes of header== *
>%%LINK%%[[#^u1jcmafew3|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The maximum amount of data that a link-layer frame can carry iscalled the maximum transmission unit (MTU)== *
>%%LINK%%[[#^4hs9wdp4icn|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==how are you going tosqueeze this oversized IP datagram into the payload field of the link-layer frame? The solution is tofragment the payload in the IP datagram into two or more smaller IP datagrams, encapsulate each ofthese smaller IP datagrams in a separate link-layer frame; and send these frames over the outgoing link.Each of these smaller datagrams is referred to as a fragment== *
>%%LINK%%[[#^atzrzdi8iep|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Fragments need to be reassembled before they reach the transport layer at the destination== *
>%%LINK%%[[#^pd0p5zwyu7q|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the designers of IP (version 4) put identification, flag, andfragmentation offset fields in the IP datagram header. When a datagram is created, the sending hoststamps the datagram with an identification number as well as source and destination addresses.Typically, the sending host increments the identification number for each datagram it sends. When arouter needs to fragment a datagram, each resulting datagram (that is, fragment) is stamped== *
>%%LINK%%[[#^jqjmbcjojd9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==A datagram of 4,000 bytes (20 bytes of IP header plus 3,980 bytesof IP payload) arrives at a router and must be forwarded to a link with an MTU of 1,500 bytes. Thisimplies that the 3,980 data bytes in the original datagram must be allocated to three separate fragments(each of which is also an IP datagram)== *
>%%LINK%%[[#^u6rzrd53bl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A host typically has only a single link into the network; when IP in thehost wants to send a datagram, it does so over this link. The boundary between the host and thephysical link is called an interface== *
>%%LINK%%[[#^7cdg7ttuinq|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==P requires each host and routerinterface to have its own IP address. Thus, an IP address is technically associated with an interface,rather than with the host or router containing that interface== *
>%%LINK%%[[#^pa00ocjs3r|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==there are thus a total of 2  (or approximately4 billion) possible IP addresses== *
>%%LINK%%[[#^o2urxw156u|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==For example, consider the IP address 193.32.216.9. The 193 isthe decimal equivalent of the first 8 bits of the address; the 32 is the decimal equivalent of the second 8bits of the address, and so on== *
>%%LINK%%[[#^0z38ijz3cht9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==this network interconnecting three host interfaces and one router interface forms a subnet== *
>%%LINK%%[[#^rqwotat6pf7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==/24 (“slash-24”) notation, sometimes known as a subnet mask, indicates that the leftmost 24bits of the 32-bit quantity define the subnet address== *
>%%LINK%%[[#^hsga4557dcb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The Internet’s address assignment strategy is known as Classless Interdomain Routing (CIDR—pronounced cider) [RFC 4632]. CIDR generalizes the notion of subnet addressing== *
>%%LINK%%[[#^07zu8drx5ar7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The x most significant bits of an address of the form a.b.c.d/x constitute the network portion of the IPaddress, and are often referred to as the prefix== *
>%%LINK%%[[#^p7nob3yj4m|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==When a host sends a datagram with destination address 255.255.255.255, themessage is delivered to all hosts on the same subnet.== *
>%%LINK%%[[#^hy5w9ycks19|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==IP addresses are managed under the authorityof the Internet Corporation for Assigned Names and Numbers (ICANN)== *
>%%LINK%%[[#^fahnfsv8zgi|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The role of the nonprofit ICANN organization [NTIA 1998] is not onlyto allocate IP addresses, but also to manage the DNS root servers== *
>%%LINK%%[[#^087kja55entv|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Host addresses can alsobe configured manually, but typically this is done using the Dynamic Host Configuration Protocol(DHCP) [RFC 2131]. DHCP allows a host to obtain (be allocated) an IP address automatically== *
>%%LINK%%[[#^elf7flnmh9q|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Because of DHCP’s ability to automate the network-related aspects of connecting a host into a network,it is often referred to as a plug-and-play or zeroconf (zero-configuration) protocol.== *
>%%LINK%%[[#^mbsj4c8qrr|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DHCP is a client-server protocol== *
>%%LINK%%[[#^mic4rfv1xa|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==The four steps are:== *
>%%LINK%%[[#^q5vrohgmuxk|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DHCP server discovery. The first task of a newly arriving host is to find a DHCP server with whichto interact. This is done using a DHCP discover message== *
>%%LINK%%[[#^3lyfkavb089|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DHCP server offer(s). A DHCP server receiving a DHCP discover message responds to the clientwith a DHCP offer message that is broadcast to all nodes on the subnet, again using the IPbroadcast address of 255.255.255.255== *
>%%LINK%%[[#^hpeup49nbc9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DHCP request. The newly arriving client will choose from among one or more server offers andrespond to its selected offer with a DHCP request message== *
>%%LINK%%[[#^4oungvqgpm7|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==DHCP ACK. The server responds to the DHCP request message with a DHCP ACK message,confirming the requested parameters.== *
>%%LINK%%[[#^93c53lrvwuu|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==From a mobility aspect, DHCP does have one very significant shortcoming. Since a new IP address isobtained from DHCP each time a node connects to a new subnet, a TCP connection to a remoteapplication cannot be maintained as a mobile node moves between subnets.== *
>%%LINK%%[[#^5ya9ooyfis9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Expanded addressing capabilities. IPv6 increases the size of the IP address from 32 to 128 bits.== *
>%%LINK%%[[#^ocyoucwbr1|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A streamlined 40-byte header== *
>%%LINK%%[[#^v4uxvy5ykh|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Flow labeling. IPv6 has an elusive definition of a flow== *
>%%LINK%%[[#^9sco31wsc3m|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==Version== *
>%%LINK%%[[#^hf4z2zqdojl|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Traffic class. The 8-bit traffic class field, like the TOS field in IPv4, can be used to give priority tocertain datagrams within a flow== *
>%%LINK%%[[#^g77jjyebrzw|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Flow label== *
>%%LINK%%[[#^rab0f8208cs|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Payload length== *
>%%LINK%%[[#^7fdnxvgzs6m|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Next header. This field identifies the protocol to which the contents (data field) of this datagram willbe delivered (for example, to TCP or UDP== *
>%%LINK%%[[#^r6uyvd1no3k|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Hop limit. The contents of this field are decremented by one by each router that forwards thedatagram. If the hop limit count reaches zero, the datagram is ­discarded== *
>%%LINK%%[[#^2t5vnipdoej|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Source and destination addresses== *
>%%LINK%%[[#^drt8a63ygcg|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>%%
>*%%HIGHLIGHT%% ==Data== *
>%%LINK%%[[#^0yvuolqj75b9|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Fragmentation/reassembly. IPv6 does not allow for fragmentation and reassembly at intermediaterouters; these operations can be performed only by the source and destination. If an IPv6 datagramreceived by a router is too large to be forwarded over the outgoing link, the router simply drops thedatagram and sends a “Packet Too Big” ICMP error message (see Section 5.6) back to the sender.The sender can then resend the data, using a smaller IP datagram size.== *
>%%LINK%%[[#^c1opyzh5blq|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Header checksum. Because the transport-layer (for example, TCP and UDP) and link-layer (forexample, Ethernet) protocols in the Internet layers perform checksumming, the designers of IPprobably felt that this functionality was sufficiently redundant in the network layer that it could beremoved.== *
>%%LINK%%[[#^iolwxtvtwzp|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Options. An options field is no longer a part of the standard IP header. However, it has not goneaway. Instead, the options field is one of the possible next headers pointed to from within the IPv6header.== *
>%%LINK%%[[#^m02k940y55|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==routing algorithms, whose goal is to determine good paths (equivalently,routes), from senders to receivers, through the network of routers. Typically, a “good” path is one that hasthe least cost== *
>%%LINK%%[[#^fbc213dz9hb|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==an edge’s cost mayreflect the physical length of the corresponding link== *
>%%LINK%%[[#^t4nwm2zmlmo|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==we’ll only consider undirected graphs (i.e., graphs whoseedges do not have a direction)== *
>%%LINK%%[[#^ku3lopdm64|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Also, a node y is said to be a neighbor of node x if (x, y) belongs toE== *
>%%LINK%%[[#^9sr92490igd|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==the least-cost path is also the shortest path== *
>%%LINK%%[[#^roe0v8dqlz|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Broadly, one way in which we can classify routing algorithms is according to whether they are centralizedor decentralized.== *
>%%LINK%%[[#^sket46k3q4n|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A centralized routing algorithm computes the least-cost path between a source and destinationusing complete, global knowledge about the network== *
>%%LINK%%[[#^j9h2kr35ovs|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Algorithms with global stateinformation are often referred to as link-state (LS) algorithms, since the algorithm must be aware ofthe cost of each link in the network.== *
>%%LINK%%[[#^51ew93hwmh3|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==In a decentralized routing algorithm, the calculation of the least-cost path is carried out in an== *
>%%LINK%%[[#^mp4bm5vd4fh|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==iterative, distributed manner by the routers. No node has complete information about the costs of allnetwork links.== *
>%%LINK%%[[#^qfvxul2z1ig|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A second broad way to classify routing algorithms is according to whether they are static or dynamic. Instatic routing algorithms, routes change very slowly over time, often as a result of human intervention(for example, a human manually editing a link costs). Dynamic routing algorithms change the routingpaths as the network traffic loads or topology change.== *
>%%LINK%%[[#^me5voyzpuaj|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==A third way to classify routing algorithms is according to whether they are load-sensitive or load-insensitive. In a load-sensitive algorithm, link costs vary dynamically to reflect the current level ofcongestion in the underlying link. If a high cost is associated with a link that is currently congested, arouting algorithm will tend to choose routes around such a congested link.== *
>%%LINK%%[[#^yjqh65qufee|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
...
>```
>%%
>*%%HIGHLIGHT%% ==Today’s Internet routing algorithms (such as RIP, OSPF, and BGP) are load-insensitive, as alink’s cost does not explicitly reflect its current (or recent past) level of congestion== *
>%%LINK%%[[#^4spjiec1bcy|show annotation]]
>%%COMMENT%%
>
>%%TAGS%%
>
